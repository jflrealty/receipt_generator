...
FONT_PATH  = os.path.join("static", "FoundersGrotesk-Regular.ttf")
LOGO_PATH  = os.path.join("static", "logo.png")
LOGO_W     = 35      # mm
LOGO_H     = 27.9    # mm (proporcional 4800×3831)
LOGO_X     = 87.5    # mm (centralizado em A4)
LOGO_Y     = 10      # mm
...
def build_pdf(i, nome, valor, unidade, data_inicio, data_fim, data_emissao):
    pdf = FPDF()
    pdf.add_page()
    pdf.set_auto_page_break(auto=True, margin=15)

    # Registra fonte customizada
    pdf.add_font('FG', style='', fname=FONT_PATH)
    ...
    pdf.set_font("FG", size=8)
    pdf.cell(0, 8, f"Receipt No.: {numero}")
    ...
    pdf.ln(15)
    pdf.set_font("FG", size=11)
    pdf.cell(0, 10, f"Resident: {nome}", new_x="LEFT", new_y="NEXT", align='L')
    pdf.cell(0, 10,
    ...
    pdf.ln(5)
    pdf.set_font("FG", size=11)
    pdf.cell(0, 10, f"São Paulo, {data_emissao.strftime('%B %d, %Y')}", new_x="LEFT", new_y="NEXT", align='L')

    pdf.ln(25)
    pdf.set_font("FG", size=12)
    pdf.cell(0, 5, "__________________________", new_x="LEFT", new_y="NEXT", align='C')
    pdf.set_font("FG", size=10)
    pdf.cell(0, 10, "JFL Administradora Imobiliária Nações Unidas Ltda.", new_x="LEFT", new_y="NEXT", align='C')
...
def gerar():
    ...
    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zipf:
        for i in range(total_linhas):
            ...
            pdf_bytes = build_pdf(i, nome, valor, unidade, data_inicio, data_fim, data_emissao)
